...
w = 8*np.pi*h*c/((lamda*1e-6)**5)*np.exp(-h*c/(k*T*lamda*1e-6))
rj = 8*np.pi*k*T/((lamda*1e-6)**4)
...
plt.plot(lamda,w, '--b', lw=2, label = "Wien")
plt.plot(lamda,rj, '--r', lw=2, label = "Rayleigh")
...
def update(val):
    ...
    ax1.plot(lamda,w1, '--b', lw=2, label = "Wien")
    ax1.plot(lamda,rj1, '--r', lw=2, label = "Rayleigh")
